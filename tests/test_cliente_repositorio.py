...
class TestClienteRepositorio(TestBase):
    ...
    def test_deve_retornar_o_extrato_do_cliente(
        self, repositorio: ClienteRepositorio, connection
    ):
        with connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO TRANSACOES (cliente_id, valor, tipo, descricao, saldo, data_transacao) VALUES (1, 100, 'c', 'descricao', 100, '2024-01-01')"
            )
            cursor.execute(
                "INSERT INTO TRANSACOES (cliente_id, valor, tipo, descricao, saldo, data_transacao) VALUES (1, 51, 'd', 'descricao', 49, '2024-01-02')"
            )

        connection.commit()

        extrato = repositorio.get_extrato(cliente_id=1)
    ...
    def test_o_numero_maximo_das_ultimas_transacoes_no_extrato_deve_ser_10(
        ...
        with connection.cursor() as cursor:
            for i in range(12):
                cursor.execute(
                    "INSERT INTO TRANSACOES (cliente_id, valor, tipo, descricao, saldo, data_transacao) VALUES (1, 51, 'd', 'descricao', 49, %s)",
                    (f"2024-01-0{i+1}",),
                )
    ...
    def test_deve_retornar_um_erro_quando_o_cliente_nao_existir(
        ...
        with pytest.raises(ValueError):
            repositorio.get_extrato(cliente_id=99)
